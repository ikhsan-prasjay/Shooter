var scenePlay = new Phaser.Class({
    ...
    create: function () {
        ...
        this.time.addEvent({delay: 250, callback: function () {
                ...
                // FIXED: memainkan sound efek tembakan hanya jika soundEnabled true
                if (soundEnabled) {
                    this.snd_shoot.play();
                }
        ...
        // FIXED: hanya mainkan background music jika soundEnabled true
        if (soundEnabled) {
            this.bgMusic.play();
        }

        // REMOVED: Baris yang salah - tidak perlu memanggil sound efek di create()
        // if (soundEnabled) this.snd_explode.play();
        // if (soundEnabled) this.snd_shoot.play();
    },
    update: function () {
        ...
        for (let i = 0; i < this.arrEnemies.length; i++) {
            ...
            for (let j = 0; j < this.arrBullets.length; j++) {
                ...
                if (this.arrEnemies[i].getBounds().contains(this.arrBullets[j].x, this.arrBullets[j].y)) {
                    ...
                    // FIXED: memainkan sound efek ledakan hanya jika soundEnabled true
                    if (soundEnabled) {
        ...
        // mendeteksi jika pesawat hero bertabrakan dengan musuh
        for (let i = 0; i < this.arrEnemies.length; i++) {
            let enemy = this.arrEnemies[i];
            // Cek apakah musuh masih aktif dan menabrak hero
            if (enemy.active && Phaser.Geom.Intersects.RectangleToRectangle(this.heroShip.getBounds(), enemy.getBounds())) {
                // FIXED: Memainkan efek ledakan hanya jika soundEnabled true
                if (soundEnabled) {
                    this.snd_explode.play();
                }

                // Menghentikan background music jika sedang bermain
                if (this.bgMusic && this.bgMusic.isPlaying) {
                    this.bgMusic.stop();
                }

                // Pindah ke scene Game Over dan kirim skor akhir
                this.scene.start('sceneGameOver', { score: this.scoreValue });
                return; // keluar dari update() untuk mencegah error lanjutan
            }
        }
    }
});